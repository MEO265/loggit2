...
`loggit` is an [`ndJSON`](https://github.com/ndjson/ndjson-spec) logging library for R software,
...
If deploying your R code in a container ecosystem, you don't even need to do that,
since `loggit` will echo its formatted logs to `stdout`. No need to write custom formatters,
handlers, levels, etc. -- ***just loggit!***
...
Or, to get the latest development version from GitHub via [devtools](https://github.com/hadley/devtools):
